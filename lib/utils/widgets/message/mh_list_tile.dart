...
class _MHListTileState extends State<MHListTile> {
  ...
  Widget _buildItemWidget() {
    return GestureDetector(
      onTapDown: (widget.allowTap && widget.enabled && !widget.selected) ? _handleTapDown : null, // Handle the tap events in the order that
      onTapUp: (widget.allowTap && widget.enabled && !widget.selected) ? _handleTapUp : null, // they occur: down, up, tap, cancel
      ...
      onLongPress: widget.enabled
          ? () {
              if (widget.onLongPressValue != null && widget.onLongPressValue is Function) {
                widget.onTapValue(context);
              } else if (widget.onLongPress != null && widget.onLongPress is Function) {
                widget.onTap();
              }
            }
          : null,
      onTapCancel: (widget.allowTap && widget.enabled && !widget.selected) ? _handleTapCancel : null,
      child: Container(
        height: widget.height == null ? null : widget.height - (widget.dividerHeight ?? 0.5),
        color: _fetchColor(),
