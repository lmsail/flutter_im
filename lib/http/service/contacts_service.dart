...
class ContactsService {
  ...
  Future fetchContacts() async {
    ...
    // 获取用户信息列表
    final jsonStr = await rootBundle.loadString(Constant.mockData + 'contacts.json');
    ...
    contactsJson.forEach((json) {
      ...
      _contactsMap[user.id] = user;
    });

    for (int i = 0, length = _contactsList.length; i < length; i++) {
      String pinyin = PinyinHelper.getPinyinE(_contactsList[i].name);
      String tag = pinyin.substring(0, 1).toUpperCase();
      _contactsList[i].namePinyin = pinyin;
    ...
    SuspensionUtil.setShowSuspensionStatus(_contactsList); // show sus tag.
